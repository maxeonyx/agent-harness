...
- **Cache figures: accept the labelling?** The ~10× read and ~1.25× write figures are unverified general-knowledge assumptions for the cache probe to check; your 5× is the conservative floor for whether the handover tool pays. Compaction designs to the conservative figure. Nothing to rule unless you disagree with the labelling.
