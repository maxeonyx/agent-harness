...
Provisional; built stage by stage per `README.md` (why → what → interactions → summary). **Stages: why #4 and #7 (user-involved, ruled 2026-08-04) · why #1–#3, #5, #6, what, interactions, summary (agent-drafted, unreviewed).** Derives from `source-notes/tech.md`, `source-notes/agent-harness-design.md`, and `source-notes/federated-brain.md`, with deferred design inputs in `experiments/event-streaming-notes.md`.
...
The claim in its honest form is narrower than "the same components with only the transport substituted". What gets substituted is the *substrate*, and substrates differ in the guarantees they offer rather than only in how bytes move. A transport is asked to promise just two things: messages on a single link arrive in the order they were sent, and every message carries a sender-assigned identity. It is not asked to promise delivery, because a link can die with messages in it — replay after reconnect is resume's job, and at-least-once delivery, deduplication and total order are all built above resume out of sequence numbers and those identities. Sequencing itself belongs to the substrate and to no participant — within one process appends are synchronous under a lock and a total order exists; across processes there is neither — so participant logic is written against the weakest substrate, and a stronger substrate is an optimisation the logic may benefit from and must never require. One logic, correct under the weakest substrate, faster under stronger ones.
...
If in-process and split deployments run different code paths, they drift, and the split path is the one that quietly rots — it is exercised less and it fails in ways local development never sees. The decoupled monolith exists to prevent that: the same components, the same messages, the same protocol, over a substituted substrate (channel, IPC, or network). Substrates differ in the guarantees they offer and not only in how bytes move, so the requirement this root imposes is precise — one logic, correct under the weakest substrate, faster under stronger ones.
...
The proposal here is that a transport promises exactly two things: messages on a single link arrive in the order they were sent, and every message carries a sender-assigned identity. It does not promise to deliver — a link can die with messages in it — and it promises nothing across links, nothing about latency, and nothing about liveness.

The layering matters, because it is easy to put delivery in the wrong place. **At-least-once is achieved above the transport, not by it:** the only way a message survives a dead link is for the sender to replay it after reconnecting, which is resume, which is built on sequence numbers. So the order is transport (per-link ordering, sender-assigned identity) → resume (replay from a sequence) → at-least-once delivery → deduplication using those same identities → total order where a substrate offers one. A transport that buffered and retried internally would be taking over resume, which is exactly the layer confusion to avoid.

The source notes are silent on this contract, so per-link ordering is a proposal, chosen because all three plausible implementations already give it for free (a channel, a stream socket, a websocket) and because taking it away buys nothing.
...
This is why #3's requirement stated as a discipline on the code rather than as an aspiration about deployments: write against the weakest substrate, and let a stronger one be a speedup you never depend on.
...
On top of that, the same scenario runs over every configuration, each with whichever substrates it can actually use — the monolith has only the in-process one, the federation chain only the network one, and the same-machine splits have both IPC and, where a link is loopback, the network. The assertion is that the durable record comes out the same — the same facts with the same causal relationships, setting aside the timings and identities that are legitimately different between two runs. If it does not, the claim that these are deployment choices over one logical model is simply false, and better to know.

### The seven configurations, and what each one is for

The list of required configurations is not a checklist someone wrote down; it is a covering set, and it reads better if you know what it covers. Each of the three role boundaries should appear both co-located and split at least once; each multiplicity the protocol must support should appear once; and one entry adds a *link* rather than moving a role.
...
`face+brain ↔ limb A, limb B` — one brain with two limbs, driving two sessions. This is the configuration the entire limb concept exists for: the source notes' own why is N projects against one brain rather than N harnesses — "Memory should scale with active work, not with every open project" — and limb-model's why #5 restates it. It is also the only fixture that exercises limb-model's undesigned port selection, whose sole forcing case is several limbs running on one host under one user. What it asserts is the absence of cross-limb interference: two sessions in two places produce two independent durable records, and neither limb's environment facts, tool declarations or process trees appear in the other's.
...
The optional direct face↔limb stream is the seventh entry and the only one that adds a link instead of moving a role, so it is treated separately below.

One honest gap. Detaching the face — the notes' hedged "If possible, we should support transitioning the process to being only a limb ... maybe re-parenting it to systemd or task scheduler" — is not a configuration at all, it is a *transition* between two of them, which makes it a distinct class of test that the list does not contain. The interactions section resolves where that belongs.
...
Federation is unevenly settled, and the section is sized to that rather than to the topic. **More than one brain is required** — the domains must be separate (why #4) — so the protocol has to carry brain-to-brain links whether or not anything else here survives. The **merged view** on top of it is a want rather than a requirement, and the section is short in proportion to that half.

The minimum content that belongs to topology is that a brain can proxy a face to another brain transparently, that the session list the user sees is merged across brains, and that a limb has a primary brain affinity which the user can override. On affinity there is a real question left, and it is worth stating precisely rather than as a general worry about N brains. The notes expect "a brain server to be running on every machine which hosts limbs", while the ruling says one brain per domain is enough and the user is "quite happy for one home brain". Those meet whenever one domain spans two machines — a desktop and a laptop, both "home". Either that domain has one brain and the laptop's limbs attach across the network, or it has two brains and something has to own the domain's billing and rate-limit view. That is the only remaining form of the N-of-everything concern, and it is asked below.
...
Topology owns the *contract and the mechanism*: the transport promise (per-link ordering plus sender-assigned identity) and the resume layer above it that turns that into at-least-once delivery, the handshake in which a joining peer declares itself, catches up and signals ready, the resume point as the highest sequence a client has consumed, and the verdict on whether same-machine IPC can be sequenced at all. It also owns the cross-configuration assertion — that the same scenario produces the same facts with the same causal relationships over every substrate.
...
One requirement arrives in the other direction, and working it out narrows it usefully. Multi-client-ui's throbber argument is that a face which has just attached must render the correct waiting-state from catch-up alone. For most of the states that is already satisfied by what this design carries: the face learns that a request was sent and that a response began, and it learns that a tool call started and exited, so model-response, tool-execution and awaiting-user are derivable from facts already crossing the boundary. What is *not* covered is the throbber's third state — harness internals, an MCP service initialising, a tool result being processed, conversation history being reordered. Nothing currently records those, so the addition is specific: **internal operations must emit start and end facts into the stream.** Recorded here rather than folded into the what, because it came from the UI side.

### The standing merge question with modular-components

Both designs asked this, so it is answered once, in `INTERACTIONS.md`: **do not merge; make modular-components a precondition of topology.** Their falsification surfaces differ, so a merged failure would be unattributable. What topology takes from the split is one obligation: modular-components owns the builder and the distinct-port-instances rule, and this design's deliberately hostile co-located configuration is the check that the rule held — only the builder can enforce distinctness, only a hostile deployment can prove it was not quietly bypassed. It wants the user's ruling, and the ruling item lives in `INTERACTIONS.md`'s questions.
...
Detaching the face resolves better once operator-lifecycle's what is read alongside this one, and it closes the one gap this design admits. Operator-lifecycle already designs the transition mechanically: a limb is a row in the brain and does not depend on the process's mode, the brain must read this as a face disconnect plus a limb continuation rather than a shutdown, and the surviving process must escape its terminal session (`setsid`, or re-parenting to Task Scheduler). So the split is: **topology owns "kill the face and the work continues, on Windows and Linux"; operator-lifecycle owns "the process sheds its face and continues as limb-only".** The first is a configuration property and belongs in the scenario suite here; the second is a lifecycle transition and does not. Read that way, the detach gap is not a missing configuration in this experiment — it is a row in operator-lifecycle's matrix.

### Layered shutdown lands on this design's boundaries

Layered-shutdown originally handed this design a "second form" of shutdown — the claim that a parent's kill authority stops at the network, so the pattern degrades into far-end self-termination. The user rejected that (2026-08-04) as an invented constraint: a layer always has kill authority over its children, exercised by command over the protocol, so only the kill command crosses the wire and each layer delivers real kills to what it locally owns. Shutdown is therefore one pattern at every scale, and what this experiment verifies about it is exactly its thesis applied to one scenario: a shutdown initiated at the face completes identically in every configuration, co-located or split, with nothing left running. The orphan timeout that *is* already ruled for remote limbs — survive an ungraceful disconnect in case of reconnect, self-terminate if nobody comes back, on the limb's own timer because a timer on a vanished brain cannot fire — is the fallback for an owner that disappeared without commanding anything, and it is a distinct row in the configurations, not the remote form of shutdown.

Layered-shutdown also concludes it has no thesis worth a standalone experiment. Taking that at face value, **topology absorbs the remote-shutdown and vanished-owner rows and operator-lifecycle absorbs the timing and exit-code rows.** That is a scoping call on somebody else's scope, so it is flagged for review rather than assumed.

### Limbs, the meta limb, and why in-process is not a test-only path

Why #6 is inherited from limb-model and the boundary follows the inheritance. Limb-model owns the limb as a stored configuration row, the factory and its construction spec, the materialisation of a remote place, and the SSH-plus-tunnel sequence including its own proposal that the limb prints its chosen port. Topology owns the transport that runs over the tunnel and the claim that a remote limb is the same logic as a local one. So this experiment assumes a limb can be made and reached, and tests only that reaching it changes nothing about the logic.

The port proposal is the one place that boundary needs a fixture rather than a handoff. Limb-model leaves port selection undesigned and names its forcing case — several limbs on one host under one user — which is precisely the `face+brain ↔ limb A, limb B` configuration above. So that configuration is where limb-model's proposal gets exercised, and it is the reason multiple limbs on one brain is in the covering set rather than left as a gap.
...
Oauth-credentials meets this design only at invariant 1, which is already settled: topology's assertion is that provider credentials are observable nowhere but the brain, and where they actually live — inside the session database as a durable-never-projected row class, ruled 2026-08-04 — is oauth's and persistence's business and does not touch the transport. The one topological fact in the ruling is replication scope: credential rows replicate only between brains of the same profile, which is the domain-separation ruling of why #4 applied to one row class.
...
One conflict involving this design is recorded in `INTERACTIONS.md` and is not resolved here: streaming responses are deferred by explicit ruling, so the direct face↔limb fast path can only be exercised for tool stdout, which is a weaker test of the capability than the design implies. It appears in the questions below.

`INTERACTIONS.md` also records the #4-versus-#7 conflict as resolved by the 2026-08-04 domain ruling, which is why it is not listed here. What survives it is narrower and lives in the federation section above: one domain spanning two machines.

## Questions for review

- Federation now splits in two, and the halves want different treatment. Brain-to-brain links are required, because the domains must be separate. The merged session list, background sync and backup-by-default are still wants. Should this experiment carry only the link and leave the merged view out entirely, or is the merged view what makes multi-brain worth testing at all?
...
- **One domain spanning two machines is the only surviving form of the N-of-everything question.** The 2026-08-04 ruling settles that "exactly one" is per domain, and that one home brain is enough. But the notes also expect a brain server on every machine that hosts limbs, and you have a desktop and a laptop that are both "home". Does that domain run one brain with the laptop's limbs attaching over the network, or two brains — and if two, what owns the domain's billing and rate-limit view?
- The transport contract proposed here — per-link ordering plus a sender-assigned identity, and nothing else, with delivery guarantees built above it by resume — is agent-proposed; the notes are silent. Is that the contract you want participants written against?
...
- **A requirement arriving from multi-client-ui:** a freshly attached face must render the correct waiting-state from catch-up alone. Provider and tool waits are already derivable from facts this design carries, but the throbber's harness-internals state is not recorded anywhere, so **internal operations have to emit start and end facts**. Which internal operations are worth an event is a judgement call I have not made — MCP init and history reordering clearly are; every small piece of reconciliation probably is not.
...
| Testing & verification | P | §Making co-location hostile, §The seven configurations, and what each one is for | |
