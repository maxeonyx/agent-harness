...
A context is a **frozen prefix plus a live tail**, and that one split explains everything here. Whatever is baked into the cached prefix — the agent's role, the limb's instructions, the AGENTS.md set, skill *descriptions*, the tool schemas and every enumerated option inside them — is cheap to have and frozen once you have it. Whatever was fetched by a tool call — loaded skill content, file reads, command output, the user's messages — is expensive per fetch and live. This design is the machinery for the consequences: things on the frozen side can only be *pointed at*, things on the live side can be *replaced* by an append, and the only operation permitted to move something across the line is a rebuild.

The reason it needs machinery at all is a squeeze. The world changes underneath a running session — the user edits a loaded skill, another agent rewrites shared instructions, git moves, hours pass — and an agent acting on a stale picture is straightforwardly wrong, not merely inefficient. In this harness that is the designed normal rather than an edge case, because user-turn and self-modification both manufacture it. But the obvious fix is illegal: rewriting the prefix breaks the cache that makes long sessions affordable at all. So the requirement is "correct the agent's understanding" and the constraint is "you may only add to the end".

The rule that resolves the squeeze is **actionability**: append a notice exactly when the agent has an action available, *within append mode*, that resolves the staleness. A changed skill is notifiable because the agent can re-read it. A changed limb is not, because you cannot read your way into being somewhere else. The same rule fixes how much a notice carries — not the shortest possible string, but *the minimum that makes the corrective action possible*, which for a skill is a path and for a changed tool schema is the whole new schema, since there is nothing else the agent could go and read that would tell it how to call the tool correctly.

Classifying every context element that way yields three categories — notifiable, notifiable-with-full-injection, and rebuild-only — plus two findings that are more than bookkeeping. Adding a tool and removing one are opposites rather than one category: a removed tool can leave its dead schema on the wire and fail informatively, while an added tool cannot be called at all until the frozen tools array is rebuilt, so a notice announcing it would be a *false* notice. And any *set* baked into the prefix is frozen along with it, which turns the wish for notices about new agent types or limbs into a declaration rule: sets that can change mid-session must not be schema enums. Declare them free-form, keep the valid values discoverable by tool call, and validate at execution time — which is the same policy the changed-schema case needs, where the wire's tools array is a possibly-stale *advertisement* and the limb's current schema is the truth.

Pushed on, the rebuild-only category turns out to be nearly empty, and the reason is worth having on its own. Referential changes — a fact the agent used is now different — append fine, because models handle "that has been updated" well. Behavioural changes — who the agent is, or where it is acting — do not, because they contradict rather than update. But a limb is identified by host plus directory and a session is bound to exactly one limb, so a changed limb or working directory is not a change to a session at all; it is a different session. A role change is unreachable the same way. A model change is not a context change either, since model is a request fact — it invalidates the cache and so lands the session in rebuild mode for free. What is actually left in the rebuild-only category is tool additions.

A notice carries four things: which element, what kind of change, who or what changed it — the user, another agent, or git, and those imply different responses — and what action is available. Notices are batched and appended at the next request, never between a tool call and its result, and they speak in the *harness's* voice rather than the user's. The mechanism that makes them behave well is to compute the notice set as a **diff at flush time** — comparing the world as it is now against what this context actually contains — rather than as a queue of change events. Then forty saves of one file produce one notice, a change-and-revert produces silence, and an idle week accumulates nothing, because there is nothing to accumulate. The price is a storage requirement: the context must record what version of what it contains, which is the same record compaction's old→new diff reads.

A notice is never a reason to call the model. Change detection piggybacks on a request that was going to happen anyway, so a file save in a session that has gone quiet costs nothing at all — under the diff formulation the notice is not even constructed. That is the difference between a design where the user can edit freely and one where saving a file costs money, and it is directly testable: save a file and observe zero requests, end a turn and observe one carrying the notice. Elapsed time works the same way, injected past roughly an hour because below that there is nothing to act on, and computed at flush rather than at detection — a notice saying two hours, delivered three days later, is worse than silence.

Rebuild is not the fallback for when notices fail. It is what you do when you were **already going to pay for a cache miss**, and at that moment there is no reason not to take everything: refresh the AGENTS.md and skill content and the schemas, canonicalise the option sets, truncate old tool output harder. So the interesting property of a rebuild is its *schedule*, set by cache expiry, model changes, handovers, and fresh sessions, and the design job is to use those moments fully rather than to make rebuild rare. Two rules constrain what it produces. It must be **canonical** — it looks as though the session had started now, and in particular it does not replay the notices that described how the superseded context went stale. And it must be **deterministic**, because compaction runs a rebuild as a dry run in order to describe a successor that does not exist yet. That schedule is also what makes rebuild-only tolerable as an answer at all, which leaves this design quietly dependent on compaction firing on time.

Progressive disclosure arrives from a different root and lands in the same machinery, which is why it belongs in the same design. The up-front context budget is paid on *every* session, including all the ones that never touch the skill or tool in question, so skills carry descriptions rather than content, descriptions say *when to load* rather than what is inside, and skills can be gated behind more broadly applicable ones. The tradeoff must not be flattened: on-demand is not simply cheaper, because a fetched skill then sits in the prefix for the rest of the session, so the mechanism moves cost from certain-and-universal to conditional-and-permanent, and it wins only when the condition is rare. That makes the balance a measurement rather than a principle. It also carries a prerequisite that is not a mechanism at all — the gates only work if the descriptions are well written — so the deliverable has a documentation half. The skill version of this is already proven in the user's OpenCode fork; the same trick applied to tools is the unproven half.

Underneath all of it sit cache semantics nobody here has measured. The two pieces this design cannot proceed without are whether the tools array participates in the cached prefix, which the whole added-versus-removed asymmetry rests on, and whether providers validate tool arguments against the advertised schema, which the changed-schema policy rests on. Both are cheap to test against a real provider and impossible to settle by reasoning, which makes them the first work rather than a risk to it.
