...
Two constraints shape it. The first is a quality bar: compaction is lossy, so the one thing that must survive is the goal and the current place. Losing those is the cardinal sin, and it is what the "handover" framing is reaching for. The second is mechanical: while the cache is warm the context must be treated as append-only, so the operation that exists to save the cache must not be the thing that breaks it. Together they rule out editing, truncating or rewriting the live context at all.

So the live context is *sealed* and the whole flow is appended to the end of it. The agent calls a first tool to declare intent. It gets a minimal acknowledgement, because a call and its result must stay adjacent on the wire. A **briefing** is then appended as its own message. The agent may call ordinary tools. Finally it calls a second tool carrying the payload — the written handover plus attachments — and the successor context is built beside the old one from that payload: a fresh canonical system prompt, the handover text, and the attachments materialised as though they had just been fetched. The predecessor's context stays in the session record intact, which is what makes retry and introspection possible. This two-tool shape is the part of the design that already works: the user's OpenCode fork implements it and the note records that it does.

The extra round trip earns itself three times over. The briefing is situation-specific — *this* successor's differences, *this* situation — so it cannot live in the system prompt, and appending it at the moment it is true is how you pay for it exactly once. The model must have read the rules before it writes, which one stage cannot arrange. And the middle step is a **cleanup slot**: the agent still has full context there, so it is the last moment at which it knows for certain which scratch files are its, which branch it made, which process it started.

The briefing exists because of the design's least obvious requirement: to write forward well you must know what the fresh context will and will not contain. It sorts the situation into three buckets. **Carried for free** — everything the successor's system prompt regenerates identically, with the instruction not to repeat any of it, because the failure mode without that instruction is an agent spending a thousand output tokens restating its own AGENTS.md. **Gone unless you keep it** — the entire conversation body, which is where the quality bar bites, and where negative results, decisions with their reasons, the current place in a plan, and the user's own wording are the things systematically under-preserved. **Changed** — the parts of the successor's context that will differ, which is what stops a handover manufacturing stale facts.

That third bucket forces something concrete: the harness performs a **dry-run rebuild** at briefing time and diffs it against the sealed context. Which in turn requires a rebuild to be pure enough to run twice with the same answer, and the diff to be taken between the same projections the request builder uses rather than between internal structures. It also makes the handover the point where all mid-session drift is reconciled — the AGENTS.md edited two hours ago, the skill another agent rewrote, the tool that appeared — whether or not anything was ever appended about it. The asymmetry matters: something that *left* the prompt must now be carried forward explicitly, while something that *changed* must not be asserted at all, because the successor will read the new version.

Attachments let the payload stop paraphrasing. Files and resources named in the second call arrive in the fresh context as ordinary tool calls in a single init step, and they are **re-executed at successor start rather than copied forward**, so the successor sees the file as it is now and an attachment can never become the stale fact the diff exists to prevent. The economics are not that attachments are cheaper — the bytes cost the same either way — but that they remove the successor's first several re-reading turns, each of which is a full cache-read pass over everything so far. The counter-pressure is permanence: anything attached is re-read on every later request, so few and load-bearing is the rule.

There are two situations, not one, and the distinction is empirical rather than tidy: a compaction that *feels* mid-task produces a successor that carries on working on a task that is already finished. So **compact-and-continue** and **compact-and-report-back** are separate flows, distinguished at the briefing, and milestone compaction is the first of them with different timing rather than a third case.

Timing is the other half of the value. The agent initiates, because the agent knows the seams, and for that judgement to be real the harness has to keep telling it the state of play — roughly how large the context is, how much cache life is left, what a compacted context would cost — which is appended and therefore free, since appending is not asking. The harness may *invite* when the cache is about to lapse and the agent has not moved; that invitation is priced at a cache hit on a context about to become a cache miss, so declining costs one cheap request. And proactive timing lets compaction happen in dead time, while a long command runs or the user is away, so a short warm context is waiting when work resumes and a lapse is a cheap miss rather than an expensive one. The arithmetic underneath: cache reads run roughly ten times cheaper than fresh input, so carrying ten times more context than a compacted version would carry costs about the same per turn, and beyond that ratio you lose money on every subsequent turn. Design to the conservative five.

One caution is load-bearing rather than a hedge. Frequency multiplies quality in whichever direction quality points, so a proactive trigger sitting on top of a handover procedure that loses things makes matters worse rather than better — which argues for demonstrating quality first and enabling the proactive trigger second. Failure is otherwise mild, because nothing was mutated: a cancelled compaction leaves the sealed context usable and still warm, and a retry re-reads the briefing from cache rather than paying for it again. The one new problem is litter, and it is misleading litter — an abandoned attempt leaves a briefing permanently in the context telling the model it was about to be replaced, and then it wasn't. The proposal is a short retraction.

Two things stay deliberately open. The model-facing **name** is unsettled, and naming is treated as an experiment parameter rather than a gap, because framing measurably changes what the agent writes — the report-back finding is already evidence of that. And the **maintained handover document**, updated through the session rather than written at the end, stays a maybe; if it exists it is best read as input to the payload rather than a replacement for it.
