...
The rule that keeps injection from becoming a loophole is that a participant on the far side of a role boundary gets its ports from its own side. Handing the **limb the brain's** filesystem root or working directory would make an in-process test pass while quietly assuming co-location, and nothing would notice until the limb was remote — which is the failure invariant 10 exists to prevent.

It is worth being exact about which boundary that is, because the obvious example is the wrong one. Invariant 10 carries an explicit exception: "a face and limb commonly DO share an environment (the user's machine), and co-located deployments may share the session state directly as the substrate." So face-and-limb sharing a filesystem is not a violation; it is a supported deployment. The boundaries where sharing is a real breach are brain↔limb and brain↔face, plus limb↔limb and face↔face where two instances of the same role must not see each other's world.

Note also what the invariant does and does not require of us. It is a *discipline* rule — never reach for role-local state, take what you need from the message — and "each participant gets distinct port instances" is a *detection technique* for breaches of it, not a restatement of it. That distinction matters for the clock: invariant 10 says no shared clock may be *assumed*, which is satisfied by never reasoning across two participants' timestamps, not by handing them clocks that disagree. In practice the in-process composition gives each participant distinct working directory, distinct environment map and distinct filesystem root, which is the same discipline topology arrives at from the deployment side and calls making co-location hostile. The two designs meet exactly here.
...
The provider wire becomes the in-process fake provider's request record, and it has to keep the property the existing suite depends on most: observing what the provider saw *between* steps, not only at the end. Today that works because the scenario harness re-reads the fake provider's log at any point it likes (`requests()` in `tests/scenario.rs`, over `requests.jsonl`), which is what proves that appends never trigger requests. A design that could only inspect the wire after the scenario finished would lose the experiment's most important assertion.
...
The falsification conditions come from `PLAN.md` and are agent-drafted, so they want confirming rather than assuming: the thesis fails if in-process tests cannot reach the same assertion surfaces without asserting internals, or if config forces components to know their construction context, or if determinism needs mock/patch seams. What *is* the user's, and what these were derived from, is the testing discipline they encode — black-box first through public surfaces, injected implementations, never mock or patch. The exit condition is the walking-skeleton scenario suite running fully in-process *and* still composable into the two-binary CLI form, plus a written comparison with deconfuse. Touches invariants 4, 8 and 10.
...
The most load-bearing is mid-scenario wire observation. The existing suite proves that appends never trigger requests by reading the fake provider's request log between steps, and that assertion is not local to this design — it is the shared instrument for invariant 2. Context-updates needs it to show that saving a file produces zero requests and that the notice then rides on a request that was happening anyway. Compaction-handover needs it to show that the harness telling an agent the state of play costs nothing extra. User-turn needs it to show that passive activity triggers nothing at all. So a design that could only inspect the wire after a scenario finished would not merely lose a convenience; it would remove the only instrument three other experiments have for their central invariant. **In-process wire observation between steps is a hard requirement of this design, not an optimisation of it.**

The durable-record handle wants one decision that this doc has not previously made. Persistence-analytics has already ruled that the *queries* are the compatibility surface rather than the table shapes, with a set of named queries checked into the repository and exercised by black-box tests. Taking that seriously, **the injected durable-record handle exposes the record's public query surface rather than its storage shape**, so this design's tests survive persistence's migrations by construction rather than by luck. That also keeps the boundary honest: reading the record through a handle is reading a product-public surface, exactly as reading the fake provider's request log is, whereas a handle that exposed tables would be an internal wearing a port's clothing.

That is a rule about the port, not a dependency on persistence. Whatever the durable record's public query surface happens to be is what the handle exposes — today the walking skeleton's journal, later persistence's named query set — and nothing about how a component is constructed changes when the surface behind the port does. So this experiment starts against what exists and inherits persistence's surface when it lands. It is worth being explicit, because the opposite reading would manufacture an ordering constraint that neither `INTERACTIONS.md` nor `PLAN.md` believes in: both record this design as depending on nothing else in the pool.
...
Both docs raise it, so the reasoning and the answer live once in `INTERACTIONS.md` rather than twice here and there. The short of it: **do not merge; make this experiment a precondition of topology**, because the two have different falsification surfaces and a merged failure would be unattributable. What this design takes from that answer is one ownership claim — **it owns the builder and the rule that each participant is handed distinct port instances**, since only the builder can enforce distinctness, while topology's deliberately hostile co-located configuration is the check that the rule held.
...
Self-modification's highest-priority why is never live-bricking, and its auto-rollback rung has a prerequisite it names honestly: you have to know what to blame. Its answer is a per-plugin Deno isolate, which denies a plugin any ambient access it was not handed — so for JS plugins the isolate carries this property directly, and the construction model is not a missing half of it but the same property enforced by a different mechanism.

Where the construction rule earns its own keep is on the **host** side, and that is worth stating because it is easy to assume the isolate covers everything. There is no isolate around a Rust component: one that reads a global, the environment or the clock can damage state it was never handed, and its fault is correspondingly hard to attribute. The rule also does work *inside* the isolate boundary, by keeping what a plugin *is* handed narrow enough to name — a plugin holding a broad handle can misuse it perfectly legitimately from the isolate's point of view. So the division is that this design owns how a plugin and its host neighbours are *constructed*, and self-modification owns the isolate, validation, quarantine and rollback.
...
- **The merge question with topology, answered in `INTERACTIONS.md`: don't merge, be its precondition.** Different falsification surfaces mean a merged failure would be unattributable, and the real overlap is the builder plus the distinct-port rule rather than a whole design. Ruling wanted, since both docs raised it.
...
- **The falsification conditions above are agent-drafted, not yours.** They came from `PLAN.md`, which credits you with the request and the testing guidelines but not with these three. They encode your discipline faithfully as far as I can tell, but they have not had your eyes on them, and neither has "whole suite well under a second".
...
- Injection makes it easy to hand a component another component's world, which would satisfy an in-process test while assuming co-location. "Each participant gets distinct port instances" is proposed as this experiment's standard, because only the builder can enforce it, with topology's hostile co-located configuration as the check that it held. Confirm — and note that the boundary to be hostile about is brain↔limb, since invariant 10 explicitly permits a face and limb to share an environment, so a hostile configuration built around face-and-limb would be testing the one case the invariant exempts.
- **The injected durable-record handle is proposed to expose the record's public query surface rather than its storage shape** — persistence's named query set once that exists, the walking skeleton's journal until then. That keeps these tests immune to persistence's migrations without creating an ordering dependency; confirm you read it the same way, since the stronger reading would block this experiment behind persistence for no benefit.
