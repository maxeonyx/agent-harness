...
**Topology** is the host for the cross-boundary form, because it owns the boundaries. The pattern degrades where the layer above has no kill authority, and the replacement — the far end self-terminating on loss of its owner — is already ruled in the notes for remote limbs, and restated from the limb's own side in limb-model, where a limb self-terminates on loneliness because a timer on a vanished brain cannot fire. So this design contributes no mechanism there; it contributes the observation that the two forms must not be conflated, since a design that assumes parent-held timeouts everywhere will quietly leave remote processes running. That belongs in topology's `face ↔ brain ↔ limb` configuration.
