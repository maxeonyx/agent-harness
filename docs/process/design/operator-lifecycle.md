...
Credentials are proposed to live outside the session database, in an OS keychain or a permission-restricted file. That is the right call for the replication reason oauth gives, and it has a consequence for this design that neither doc currently covers: the snapshot-before-migrate discipline, the schema version comparison, and the downgrade barrier all apply to the session database and to nothing else. A credential store with its own format is a fourth durable thing that an update can break, and nobody owns its versioning. It is small — one file, a handful of fields — which is an argument for saying so explicitly rather than for ignoring it. Raised in the questions.
...
- **Should this experiment absorb layered-shutdown's timing rows?** Layered-shutdown's own conclusion is that it has no standalone thesis, and this design is what reads the exit code and depends on the durable write landing inside the deadline. The proposal above is that the deadline and reserve discipline plus the exit-code distinction are verified here, while the cross-boundary self-termination form goes to topology. That is a scoping call, not a design one, and it is yours.
- **The credential store has no version, snapshot or migration story.** Keeping credentials out of the session database is right for the replication reason, but it means a fourth durable thing exists that an update can break, and none of this design's ceremony covers it. Should it get a version field and the same refuse-clearly treatment, or is it small enough to treat as disposable and re-authenticate on failure?
