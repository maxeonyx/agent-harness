...
A consequence decides a lot of structure: a user tool is not a component in one role, it is a triple spread across all three. The face owns the interactive surface, the limb owns execution, the brain owns what reaches the model, and what binds them is the fact stream rather than a shared object. It follows — proposed here, not ruled — that reads and writes go through the limb even where the face and limb share the user's machine, because otherwise a remote limb cannot work at all, because the limb is authoritative over truncation and diagnostics, and because the limb is what records execution facts. What stays face-local is the editing buffer: cursor, scroll, unsaved text.

What the model's projection should contain is "the same important information, minus the noise", and that splits into three rules. **Keep what informed the decision** — three files opened and one edited projects as three files, because the agent will need the understanding the user built, and this is what makes these projections different from a diff. **Drop intermediate states**, but only where the later state supersedes the earlier one entirely: fifteen saves collapse into one diff, while a command that failed and taught the user something survives, because the correction is sometimes the informative part. **Drop visual chrome** — frames, colours, scroll offsets, pane layout. The projection is also **live**: the fact that a file is open right now is actionable, and that is what turns mutual observation into something the agent can use mid-task rather than only in retrospect. One test says whether a projection is good enough — could the agent answer "what did the user look at, and what did he change?" without asking?

The tools are a file explorer and editor, a persistent terminal, search, GitHub, and a subagent. Two details in them are load-bearing rather than cosmetic. Files opening **collapsed** is the mechanism that makes the file projection worth anything: it turns each expansion into an observable act of attention, so the projected signal is the set of regions the user chose to look at rather than the useless fact that he had two thousand lines on screen. And search should preserve *which results he then opened*, because the path taken through nine hits is the reasoning, where the query and the hit list are only the outcome.

The GitHub tool generalises into the integration policy for every third-party interactive tool this harness might host, which is worth more than getting `gh` right: **a tool we cannot observe cannot own a projection, and a tool without a projection is not a user tool** — it is just a program the user ran. So, in order of preference: drive the tool's machine-readable surface under our own UI, host its interface and scrape it, or fork it so it emits facts. The subagent-as-user-tool is the only user tool that spends model tokens, and it does not breach the no-trigger invariant, because that invariant's word is *passive*: the user launching a child is not passive activity, the requests belong to the child session, and what lands in the parent is a prompt plus a result, appended.

On the wire, user activity is not assistant tool calls. It is a user message composed of several parts — the user's typed text, and clearly labelled blocks describing his activity, in the order things happened — because providers allow text in a user message rather than arbitrary structured blocks, and because the interleaving is what makes the reasoning trail legible: he said this, then looked at that, then said this. Activity may piggyback mid-tool-loop, arriving after a tool result rather than at turn end, so one episode of user work may be split across several user messages. Staged-but-unsent text does *not* piggyback, and the split is principled: activity is a fact about the world that is useless if it arrives late, while staged text is deliberate composition the user has not chosen to send.

Getting in and out is by sigil — `$` for the terminal, `@` for the file tool, search still undecided — under the constraint that `$` must stay typeable in an ordinary message. Submission has three states rather than two: shift+enter inserts a newline while drafting (which depends on the kitty keyboard protocol, a real terminal requirement rather than an incidental detail), enter with content **stages** a part and empties the buffer, and enter with an empty buffer sends everything staged, as does ctrl+enter where the terminal can deliver it. So pressing enter twice sends, which matches chat muscle memory while making it structurally impossible to fire off a half-typed line by reflex. Staging is more than convenience: since a user turn is several message parts and the model probably sees them separately, staging is how the user authors that part structure — the keyboard end of the same decision as the wire form.

Conflicts get one mechanism and deliberately nothing more. **Writes carry the version they were based on**, and are accepted only if the file still has that content hash. That compare-and-swap rejects only genuine lost updates, permits the agent writing to a file the user merely has open, needs no locking or presence protocol, and delivers "the user wins" as a consequence rather than as a special rule — it is the agent's write that is based on the stale read. A rejected write returns the fact that the file changed and what changed, which turns a conflict into a well-framed context update rather than an error. Non-file conflicts are not covered, and mutual observation is the mitigation, which is exactly the restraint the notes ask for: don't overprescribe live collaboration.

The concept is not a TUI feature. A UI projection is a **description of state**, not a sequence of terminal escapes, so a renderer realises it — TUI now, web later — and the model's projection is entirely unaffected by which client the user is sitting in front of. That boundary costs almost nothing now and is close to unaffordable to retrofit, because retrofitting means unpicking escape sequences from tool logic. Voice, if it happens, is another fact stream with a projection, timestamped and interleaved with the rest, which is a good sign for the contract rather than a new mechanism.

Two things close it off. The agent must know these tools are not its own — from self-describing activity blocks, and from the system prompt saying so — and the useful half of saying so is not "these are not yours" but "these are his, and you may ask him to use them", which is mutual observation paying off in its smallest possible form. And the assumption most exposed in the whole design is "input is cheap": generous projections are re-read on every subsequent request for the rest of the session, so their generosity is a tunable whose default should come from measurement rather than from argument. The remaining thing only the user can judge is whether the three-state input actually feels right in the hand.
...
That has one sharp practical edge. Invariant 10's by-design exception says a face and a limb commonly *do* share an environment — the user's machine — which makes it tempting to have the face's editor read and write files directly. It should not. Reads and writes go through the limb even when the face could do them locally, for three reasons: a remote limb otherwise cannot work at all; the limb is authoritative over truncation, diagnostics, and what context reaches the model from its domain; and the limb records the execution facts (the walking-skeleton ruling on split tool-fact recording). What stays face-local is the *editing buffer* — the unsaved state, the cursor, the scroll position — which is face-local UI state and, where it needs to be shared between clients, multi-client-ui's problem rather than this design's. This ruling is mine, not the notes'; see Questions for review.
...
**Keep what informed the decision.** This is why #4, and it is the rule that makes these projections different from a diff. If the user opened three files and edited one, the projection carries all three, because the agent will in theory need the same understanding the user built. Why #4's own justification is that "input is cheap", which is the licence to be generous here — but note that it is generous relative to *content*, not relative to noise, and the permanence arithmetic still applies: whatever is appended is re-read on every subsequent request for the rest of the session.
...
Pressing enter twice sends. That matches the muscle memory of every chat application, while making it structurally impossible to send a half-typed line by reflex — which is the actual complaint behind "enter would not automatically send the message". The user gets the familiar gesture and the protection at once.
...
### Concurrency: a compare-and-swap, and nothing more
...
`REQUIREMENTS.md` sets the floor from the other side — invariant 7 and the worker's needs: "the user wins on conflicting edits, and stale agent output never silently overwrites newer user work." So the design has to satisfy a hard rule with a light touch, and there is exactly one mechanism that does both.

**Writes carry the version they were based on.** The agent read the file at some content hash; when it writes, the write is accepted only if the file still has that hash. That is a compare-and-swap, and it has precisely the properties both sides want. It rejects only *real* conflicts — a genuine lost update, where the agent is about to overwrite a change it never saw. It permits everything else, including the agent writing to a file the user merely has open but has not changed, which is the case an eager rule would block for no benefit. It requires no locking, no presence protocol, no live-collaboration model. And "the user wins" is not a special rule bolted on; it falls out, because it is the agent's write that is based on the stale read.
...
Two cases the notes do not cover, recorded rather than invented. **Non-file conflicts**: two writers on the same terminal, or the agent running a command that undoes what the user just did, are not addressed by content hashing and are probably not addressed at all in this design — mutual observation is the mitigation, which is precisely the "don't overprescribe" position. And **the reverse direction**: whether the *user's* write should ever be rejected because the agent changed the file. Invariant 7 says the user wins, so no — but "the user wins" and "the user is never told" are different things, and his editor showing him a file the agent has since modified is a real situation. That is multi-client-ui's problem more than this design's, but it is worth not assuming it away.
...
It is falsified if: the agent gets confused or alarmed by user activity rather than absorbing it, which is why #1's own failure story surviving the fix; the agent asks for something it was already shown, meaning the projection is too lossy; the projections are large enough to derail attention or cost, meaning they are too generous — and note that why #4's "input is cheap" is the assumption most exposed here, since appended activity is re-read on every later request; passive activity triggers a request, contradicting invariant 2; the conflict rule either blocks the agent routinely or lets a user edit be lost; the agent tries to use the user's tools; or the three-state input scheme turns out to be confusing in the hand, which only the user can determine and which is why `PLAN.md` records that this experiment "requires a lot of hands on from me tho".
...
- **User tools execute through the limb, never directly from the face** — even though the face and limb usually share your machine. My reasons: remote limbs, limb-owned truncation and diagnostics, and limb-recorded execution facts. The cost is that an interactive editor over a remote limb is a harder problem. Is that the right trade?
...
- **Compare-and-swap on writes as the whole conflict story.** It satisfies invariant 7 ("the user wins") without any live-collaboration model, and it rejects only genuine lost updates. It does nothing for non-file conflicts, which I think is correct given "don't overprescribe", but it is worth confirming you are happy for that to be uncovered.
...
| Security | P | §Concurrency: a compare-and-swap, and nothing more | |
