...
This is a measurement, not a design: we do not know whether a provider bills for a response we stopped wanting, and exactly one decision rests on the answer — whether the harness cancels an in-flight request or lets it complete and discards the result. The hard part is not the cancelling but the instrument, because usage arrives in response metadata at the end of a stream and a cancelled stream may never deliver it, so the harness's own reporting is blind in precisely the case of interest; that forces account-level usage as the measure, which in turn forces isolated windows per condition and lets the instrument's granularity — not statistical taste — set how many repetitions are needed, landing the whole thing somewhere between roughly five and fifty dollars. The one output that must be right whatever the billing turns out to be is a **recording rule**: a cancelled request's cost may be genuinely unknown, so it must be storable as unknown-with-a-reason rather than as zero, with cost queries reporting coverage alongside their totals. That rule's shape is known already and does not depend on the measurement, so persistence can adopt it now — which makes this experiment genuinely optional, schedulable whenever there is money to spend on it and deferrable indefinitely without blocking anything.

## Why

### 1. One design decision rests on the answer — *cost, as the subject rather than as a driver*

Cost is a false *driver* in this project, but this doc's question *is* a billing question — it comes straight out of `source-notes/analytics.md` — so cost is its honest root, and naming it anything else would be evasion.

The decision that rests on it is narrow and worth stating precisely: **when the harness stops wanting a response, does it cancel the request or let it complete and discard the result?** The user's note above leans toward letting it complete, and says why — he thinks providers still charge for cancelled requests — while marking the whole thing "probably worth experiment."

If a cancelled request is billed in full regardless, discarding a response is pure waste and the harness should always let it finish and keep the result. If cancellation genuinely stops generation and stops billing, the calculus flips for long thinking responses, and there is a real saving available at the cost of losing partial work.

So the root is: **we are making one cost-shaped decision on an unverified belief about provider behaviour, and the belief is cheap to test.**

Worth being careful about what does *not* rest on it, because it is tempting to inflate this experiment's reach. Cancellation is designed here as soft — a message to the agent rather than a kill, deliberately spending tokens because cleanup is the point — which follows from "cancellation deliberately *spends* tokens so agents can clean up" (`README.md`) and is a correctness position, settled in forked-subagents on correctness grounds. The exact framing text of that message is ours to choose and appears in no note. Invariant 9's ruling that completed work tying with a cancel is *kept and recorded* has two halves — "it cost money **and is probably good**" — and the second half survives any billing answer, so that ruling does not flip either. This experiment quantifies a spend; it does not authorise or forbid one.
...
### 3. Operator lifecycle is *not* a second dependency — *recorded because it looks like one*

It is natural to add graceful shutdown to the list: it waits for in-flight API requests rather than cancelling them, which sounds like the same bet on billing. It is not, and saying so keeps this experiment honestly small.

The user's reason for waiting is continuity, not money. From `source-notes/tech.md`: "I wonder if we'd wait for all API requests to complete first before closing. Yes - that makes good sense for graceful shutdown - wait for all API requests to complete, remember for later that we're about to run tool calls or whatever, then we re-launching we run those tool calls etc. and continue where we left off." The response is what carries the tool calls the relaunch has to continue from, so cancelling it loses work regardless of what it cost. Even if cancellation turned out to be free, shutdown would still want to wait.

So a positive result here makes shutdown *cheaper*, not faster or different, and the sequence is unaffected. Operator-lifecycle currently justifies the wait on billing grounds too; that justification should be corrected there rather than reinforced here.
...
**Operator-lifecycle** takes a parameter and not a verdict, per why #3. Shutdown waits for in-flight requests because the response carries the tool calls a relaunch must continue from, which is a continuity reason that no billing answer touches. So this measurement tells that design what its wait *costs*; it does not tell it whether to wait. Operator-lifecycle currently justifies the wait on billing grounds, and that is the one thing here that wants correcting in a doc this one does not own.
...
So this design decides no portfolio-level conflict, which is why `INTERACTIONS.md` lists it among the periphery rather than among the disagreements. It supplies one parameter and one recording rule, and the rule is adoptable without it.

## Questions for review

- Is this worth doing at all, given that on inspection it changes exactly one decision — cancel versus let-it-finish — or is it fine to keep the conservative "let it finish" behaviour indefinitely and never need the answer? I originally counted three dependents here; two dissolved (shutdown wants continuity, not a billing answer, and the recording rule is independent), so the honest case for running this is weaker than it first looked.
