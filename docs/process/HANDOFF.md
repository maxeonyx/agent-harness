...
What this is: the entry point for a fresh agent — current position and pointers only. History lives in git and the experiment outcome docs; never accumulate chronology here.
...
- The forked-subagents experiment is **built and measured, awaiting Gate 1** — `experiments/forked-subagents/`, outcome `experiments/forked-subagents-outcome.md` (under `docs/process/`). Max still has to run it himself.
...
- `tests/gatekeeper.rs` still says "spikes" in its message; fold the wording fix into the next code-touching change (it needs a version bump per the CI release guard, not worth one alone).
