...
const RUN: Datum[] = [
  ...
  { id: "root", t: "the root's first messages", s: "[system, user: the task]", at: excerpt("../src/session.rs:/messages: vec![Message::new(\"system\"/") },
  { id: "before", t: "an agent's messages, turn n", s: "Vec<Message>, only ever appended to", at: excerpt("../src/wire.rs:/pub struct Message {/") },
  { id: "request", t: "request body", s: "envelope + messages + 3 tools", at: excerpt("../src/wire.rs:/pub struct ChatRequest {/") },
  { id: "provider", t: "OpenRouter → Bedrock / Azure", s: "cache read, then generation", ext: true },
  { id: "reply", t: "reply + usage", s: "assistant message, tokens, cost", at: excerpt("../src/wire.rs:/pub struct ChatResponse {/") },
