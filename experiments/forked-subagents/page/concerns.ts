...
const HANDLED: Handled[] = [
  ...
  { concern: "Cancellation", how: "one token for the run; nothing new starts, requests in flight are awaited and kept, every agent ends with an outcome, then the process closes; a second Ctrl-C force-cancels, abandoning what is in flight and still writing the record", at: excerpt("../src/agent.rs:/if run.cancel.is_cancelled() {/"), tests: ["cancel_starts_nothing_new_keeps_what_is_in_flight_and_ends_every_agent", "cancelling_stops_the_retries", "a_second_interrupt_exits_without_waiting"] },
